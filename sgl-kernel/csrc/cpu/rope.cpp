...
std::tuple<at::Tensor, at::Tensor> rope_kernel_impl(
    ...
    auto emb_pos_ptr = t_emb_pos.data_ptr<T>(); // [MP][1][HR]
        ...
        for (int n = 0; n < N; n++) {
            ...
            if (n < NK) {
                for (auto h = 0; h < HK; h += 2) {
                    ...
                    auto out1_k = in1_k * cos - in2_k * sin;
                    auto out2_k = in2_k * cos + in1_k * sin;
