...
struct RopeParams {
  ...
  // Contiguous packed [B, S, H, D] output offsets (out-of-place path).
...
struct RotaryEmbedInternal<scalar_t, RotaryMode::Interleaved> {
  static inline void apply(
    ...
    const int64_t half_size = size / 2;
...
template <typename scalar_t, RotaryMode mode, typename CachePos, bool inplace>
...
std::tuple<at::Tensor, at::Tensor> rotary_embedding_cpu(
  ...
  TORCH_CHECK(query.dim() == key.dim(), "query and key must have the same rank");

  int64_t rotary_dim = cos_sin_cache.size(1);
  TORCH_CHECK(rotary_dim <= head_size, "rotary_dim must be <= head_size, got ", rotary_dim, " vs ", head_size);
  ...
  if (input_dim <= 3) {
    CHECK_EQ(query.size(0), positions.numel());
    CHECK_EQ(key.size(0), positions.numel());
  }

  at::Tensor query_out;
  at::Tensor key_out;
  const bool inplace = (input_dim != 3);
  if (inplace) {
    query_out = query;
    key_out = key;
  } else {
    // Contiguous packed output so q_out_offset / k_out_offset are valid.
    query_out = at::empty(query.sizes(), query.options());
    key_out = at::empty(key.sizes(), key.options());
  }
  ...
  AT_DISPATCH_REDUCED_FLOATING_TYPES(input_dtype, "rotary_embedding_cpu", [&] {
    ...
    const int64_t rd = p.rotary_dim;
    auto cache_pos = [cache_base, pos_ptr, rd](int64_t token) -> const scalar_t* {
      return cache_base + pos_ptr[token] * rd;
    };
    ...
    if (inplace) {
      if (is_neox) {
        rotary_embedding_kernel_impl<scalar_t, RotaryMode::Neox, decltype(cache_pos), true>(
            q_out_ptr, k_out_ptr, q_ptr, k_ptr, p, cache_pos);
      } else {
        rotary_embedding_kernel_impl<scalar_t, RotaryMode::Interleaved, decltype(cache_pos), true>(
            q_out_ptr, k_out_ptr, q_ptr, k_ptr, p, cache_pos);
      }
    } else {
      if (is_neox) {
        rotary_embedding_kernel_impl<scalar_t, RotaryMode::Neox, decltype(cache_pos), false>(
            q_out_ptr, k_out_ptr, q_ptr, k_ptr, p, cache_pos);
      } else {
        rotary_embedding_kernel_impl<scalar_t, RotaryMode::Interleaved, decltype(cache_pos), false>(
            q_out_ptr, k_out_ptr, q_ptr, k_ptr, p, cache_pos);
      }
    }
  });
...
std::tuple<at::Tensor, at::Tensor> multimodal_rotary_embedding_cpu(
  ...
  if (positions.dim() == 2) {
    ...
    AT_DISPATCH_REDUCED_FLOATING_TYPES(input_dtype, "rotary_embedding_cpu", [&] {
      ...
      if (is_neox) {
        rotary_embedding_kernel_impl<scalar_t, RotaryMode::Neox, decltype(cache_pos), true>(
            q_ptr, k_ptr, q_ptr, k_ptr, p, cache_pos);
      } else {
        rotary_embedding_kernel_impl<scalar_t, RotaryMode::Interleaved, decltype(cache_pos), true>(
            q_ptr, k_ptr, q_ptr, k_ptr, p, cache_pos);
      }
