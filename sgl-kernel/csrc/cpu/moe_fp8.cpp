...
void shared_expert_fp8_kernel_impl(
  ...
  // stage 0: dequantize packed weights to scalar_t, [2N, K]
  at::parallel_for(0, 2 * N, 0, [&](int64_t begin, int64_t end) {
    for (int64_t n = begin; n < end; ++n) {
      dequantize_row_fp8<scalar_t>(
          Bdq_tmp + n * K,
          w1s + n,
          packed_w1 + n * K,
          K);
  ...
  // K and N are packed for fp8
  const int64_t packed_K = get_row_size<c10::Float8_e4m3fn>(K);
  const int64_t packed_N = get_row_size<c10::Float8_e4m3fn>(N);
  const int64_t stride_n = packed_K;
  ...
  at::parallel_for(0, MB * NB, 0, [&](int64_t begin, int64_t end) {
    ...
    for (int64_t i = begin; i < end; ++i) {
      ...
      // B shape [K, n_size] in vnni format
      const scalar_t* __restrict__ B0 = Bdq_tmp + nb0 * BLOCK_N * stride_n;
      const scalar_t* __restrict__ B1 = Bdq_tmp + nb1 * BLOCK_N * stride_n;
  ...
  // // stage 1.5: dequantize packed_w2 to scalar_t, [K, N]
  at::parallel_for(0, K, 0, [&](int64_t begin, int64_t end) {
  ...
  at::parallel_for(0, MB2 * NB2, 0, [&](int64_t begin, int64_t end) {
    ...
    for (int64_t i = begin; i < end; ++i) {
      ...
      // B shape [IC, n_size] in vnni format
      const scalar_t* __restrict__ B = Bdq_tmp + nb * BLOCK_N * stride_oc;
...
#define INSTANTIATE_SHARED_EXPERT_INT8_TEMPLATE(TYPE)                                        \
  template void shared_expert_fp8_kernel_impl<TYPE> (                                       \
      TYPE* __restrict__ output, TYPE* __restrict__ ic1,                                     \
      float* __restrict__ C_tmp, TYPE* __restrict__ Bdq_tmp,                               \
      float* __restrict__ Bs_tmp, const TYPE* __restrict__ input,                            \
      const at::Float8_e4m3fn* __restrict__ packed_w1, const at::Float8_e4m3fn* __restrict__ packed_w2,            \
      const float* __restrict__ w1s, const float* __restrict__ w2s,                          \
