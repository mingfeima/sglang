...
class DeepseekV2AttentionMLA(nn.Module):
    ...
    def forward_absorb_fused_mla_rope_cpu(
        ...
        qkv_proj_with_rope_is_int8 = self.qkv_proj_with_rope_is_int8
        q_input, k_input, v_input = sgl_kernel.cpu.qkv_proj_with_rope(
            ...
            kv_a_layernorm.variance_epsilon,
            use_int8_w8a8=qkv_proj_with_rope_is_int8,
            q_a_proj_scale=(
                q_a_proj.weight_scale if qkv_proj_with_rope_is_int8 else None
            ),
            q_b_proj_scale=(
                q_b_proj.weight_scale if qkv_proj_with_rope_is_int8 else None
            ),
            kv_a_proj_scale=(
                kv_a_proj_with_mqa.weight_scale
                if qkv_proj_with_rope_is_int8
                else None
            ),
        )
        attn_output = self.attn_mqa(q_input, k_input, v_input, forward_batch)
        ...
        w_vc = self.w_vc
        w_vc_dtype = w_vc.dtype
        w_scale = self.w_scale
        if w_vc_dtype == torch.float8_e4m3fnuz:
            # TODO(kernel): add bmm_fp8 for torch.float8_e4m3fnuz
            attn_bmm_output = torch.bmm(
                attn_output.to(torch.bfloat16).transpose(0, 1),
                w_vc.to(torch.bfloat16) * w_scale,
            )
        elif w_vc_dtype == torch.float8_e4m3fn:
            attn_output_val, attn_output_scale = input_to_float8(
                attn_output.transpose(0, 1), torch.float8_e4m3fn
            )
            attn_bmm_output = bmm_fp8(
